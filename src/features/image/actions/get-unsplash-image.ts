...
export const getUnsplashImage = async (query: string, page: number) => {
  ...
  try {
    const response = await fetch(
      `${import.meta.env.VITE_UNSPLASH_URL}/search/photos?client_id=${import.meta.env.VITE_UNSPLASH_ACCESS_KEY}&query=${encodeURIComponent(query)}&page=${page}&per_page=12`,
      {
        method: "GET",
        headers: {
